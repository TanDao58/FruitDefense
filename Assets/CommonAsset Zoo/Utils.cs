...
namespace DarkcupGames
{
    ...
    public static class Utils
    {
        ...
        public static List<T> RandomElement<T>(this List<T> list, int amount)
        {
            if (list.Count < amount)
            {
                Debug.LogError("not enough amount to random");
                return list;
            }
            List<T> src = new List<T>();
            src.AddRange(list);

            List<T> result = new List<T>();
            for (int i = 0; i < amount; i++)
            {
                var rand = src.RandomElement();
                result.Add(rand);
                src.Remove(rand);
            }
            return result;
        ...
        public static void UpdateSelected<T>(this List<T> list, int index, Action<T> notSelected, Action<T> selected)
        {
